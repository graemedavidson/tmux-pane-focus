  ...
  for pane in ${panes}; do
    ...
    if [[ "${active}" -eq 1 ]]; then
      ...
      if [[ "${top}" -ge "${prev_top}" ]] && [[ "${bottom}" -le "${prev_bottom}" ]]; then
        unset 'inactive_height_parent_panes[${prev_i_height}]'
        # prev_top="${bottom}"
...
if [[ "${resize_width}" == "true" ]]; then
  for pane in ${panes}; do
    ...
    # echo "previous: L:${prev_left} R:${prev_right} ID:${prev_i_width}"
    if [[ "${active}" -eq 1 ]]; then
      resize_width_panes+=("${index}")

      if [[ "${left}" -ge "${prev_left}" ]] && [[ "${right}" -le "${prev_right}" ]]; then
        unset 'inactive_width_panes[${prev_i_width}]'
        # prev_left="${right}"
        # prev_right=$(( right + 1 ))
        # prev_i_width="${index}"
      fi
    ...
    echo "new: L:${prev_left} R:${prev_right} ID:${prev_i_width}"
    echo "============"
...
for pane_index in "${!inactive_width_parent_panes[@]}"; do
  if [[ "${inactive_width_parent_panes[${pane_index}]}" -gt 1 ]]; then
    ((parent_width_parent_panes=parent_width_parent_panes+1))
  fi
done

# Remove actie pane from count
inactive_height_parent_panes="$(( ${#resize_height_panes[@]} - 1))"
...
echo ""
echo "resize height (horizontal) - inactive panes: ${inactive_height_parent_panes}, parent panes: ${parent_height_panes}"
echo "================="
...
inactive_height_parent_panes="$(( inactive_height_parent_panes - parent_height_panes))"
inactive_width_panes="$(( inactive_width_panes - parent_width_panes))"

IFS=- read -r min_inactive_height< <(get_inactive_pane_size "${window_height}" "${active_percentage}" "${inactive_height_parent_panes}")
...
if [[ "${resize_height}" == "true" ]] && [[ "${resize_height_setting}" == "true" ]]; then
  for pane_index in "${resize_height_panes[@]}"; do
    if [[ "${pane_index}" == "${active_pane_index}" ]]; then
...
if [[ "${resize_width}" == "true" ]] && [[ "${resize_width_setting}" == "true" ]]; then
  for pane_index in "${resize_width_panes[@]}"; do
    if [[ "${pane_index}" == "${active_pane_index}" ]]; then
      ...
      if [[ "${inactive_width_panes[${pane_index}]}" -gt 1 ]]; then
